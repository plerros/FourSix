...
void triangulation_t::steiner_circumcenter(bool first_run)
{
	std::vector<Point> steiner_pts;
...
void triangulation_t::steiner_mixed(unsigned int retries)
{
	...
	for (unsigned int i = 0; i < retries; i++) {
		...
		for (int j = 0; j < 20; j++) {
			...
			current2.steiner_circumcenter(true);
...
void triangulation_t::steiner_mixed_recursive(unsigned int depth, bool first_run)
{
	...
	for (int method = st_circumcenter; method <= st_random; method ++) {
		...
		switch(method) {
			case st_circumcenter:
				current.steiner_circumcenter(first_run);
				break;
			case st_projection:
				current.steiner_projection();
				break;
			case st_centroid:
				if (first_run && (best.obtuse != this->obtuse))
					break;
		...
		if (current.obtuse != this->obtuse) {
			current.steiner_mixed_recursive(depth - 1, first_run);
			if (first_run && (current.obtuse < best.obtuse))
				best = current;
			if (!first_run && (current.obtuse <= best.obtuse))
				best = current;
